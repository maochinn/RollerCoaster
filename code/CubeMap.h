...
private:
		...
		const char* front)
	{
		...
		for (int i = 0; i < 6; i++)
		{
			...
			if (img.channels() == 3)
				glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB8, img.cols, img.rows, 0, GL_RGB, GL_UNSIGNED_BYTE, img.data);
			else if (img.channels() == 4)
				glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA8, img.cols, img.rows, 0, GL_RGBA, GL_UNSIGNED_BYTE, img.data);
